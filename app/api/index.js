...
async function routeAI(message, model) {
  ...
  if (t === "/status" || t === "status" || t === "version") {
  ...
  if (model === "openai" && OPENAI_KEY) return callOpenAI(message);
  if (model === "gemini" && GEMINI_KEY) return callGemini(message);
  if (model === "groq" && GROQ_KEY) return callGroq(message);
...
module.exports = async function handler(req, res) {
  ...
  if (url.includes("/api/health")) {
    res.status(200).json({ ok: true, service: "ultron-backend", version: "v2.8", groq: GROQ_KEY ? "OK" : "NO", timestamp: new Date().toISOString() });
    return;
  }
  ...
  if (req.method === "POST" && url.includes("/api/chat")) {
    const body = req.body || {};
